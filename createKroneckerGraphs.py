...
for threads in [1]:
  for vertices in range(5,23):
     ...
     for edges in [4, 8, 16, 32]:
        ...
        os.system("%(top)s/graphbench/data/generator/generator_seq %(zero)s%(vertices)i -e %(edges)i -o %(top)s/Tracking/outputs/kroneckerGraphs/KronGraph%(zero)s%(vertices)i_%(edges)i_%(threads)i_Thesis.txt" %locals())
